...
import pickle
import csv
from sympy import Matrix, Integer, symbols, Poly, Add, eye, rem
...
def matrix_modular_exponentiation(A: Matrix, n: int = 21):
    x = symbols('x')
    A_charpoly = A.charpoly(x)
    p = Poly(x, x)
    for _ in range(n):
        p = Poly(rem(p**2, A_charpoly.as_expr()), x)

    expr = p.as_expr()
    first_term = expr.args[0] * eye(A.shape[0]) 
    modified_expr = Add(first_term, *expr.args[1:])
...
def calculate_single_cached(args, V, L):
    try:
        ...
        path_num = matrix_modular_exponentiation(Matrix(A))
...
if __name__ == "__main__":
    ...
    with open(log_file, "w", newline="") as csvfile:
        ...
        for n in range(1, 5):
            ...
            filename = f"total_sum_poly_{n}.pkl"
